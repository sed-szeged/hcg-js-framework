...
def _run_dynamic(tools_dir, filter, node_cg, node_bin, cg_path, module_name, wd):
    print('Running npm test')
    my_env = os.environ.copy()
    my_env["PATH"] = node_cg + os.pathsep + my_env["PATH"]
    my_env["CALLCHAIN_INCLUDE"] = '|'.join(filter)
    subprocess.run(['npm', 'test'], env=my_env)
    subprocess.run([os.path.join(tools_dir, 'process-results.sh'), 'v8', '-cg', tools_dir, node_bin])
    if not os.path.exists(os.path.join(cg_path, "dyn-v8")):
        os.mkdir(os.path.join(cg_path, "dyn-v8"))
    shutil.move("full-trace.json.txt", os.path.join(cg_path, "dyn-v8", module_name + ".json"))
    _change_path(os.path.join(cg_path, "dyn-v8", module_name + ".json"), wd)
...
def main(skip_clone=False):
    ...
    for node_module in CONF['modules']:
        ...
        _run_dynamic(td, node_module['filter'], node_cg, node_bin, cg_path, node_module['name'], wd)
