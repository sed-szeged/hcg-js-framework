var util = require('util'),
	...
	jsonHandler = require('./lib/jsonhandler');
...
var instrumented_files = [];

/**
 * Creates a new instance of NJSTrace
 * @class The main class that is responsible for the entire hybrid-metric-framework functionality
 ...
 * @example
 * hybrid-metric-framework.on(NJSTrace.events.error, function() {...});
 */
NJSTrace.events = {
	error: 'error',
	warn: 'warn'
...
/**
 * Start the hybrid-metric-framework instrumentation process
...
NJSTrace.prototype.inject = function(config) {
	...
	this.log('hybrid-metric-framework done loading...');

	// Save all JSON files
	jsonHandler.flushAll();
	return this;
};
...
NJSTrace.prototype.log = function() {
	...
	this.logger.print('hybrid-metric-framework: ');
...
NJSTrace.prototype.hijackCompile = function () {
	...
	Module.prototype._compile = function (content, filename) {
		...
		function loop() {
			return instrumented_files.includes(filename);
		}

		function dont() {
			self.log(filename, 'already instrumented, SKIPPING...');
			return origCompile.call(this, content, filename);
		}

		if (instrument) {
			if (loop()) { return dont.call(this); }

			instrumented_files.push(filename);
			...
			if (Module.wrapper.length === 2) {
				...
				wrapped = false;
				self.log('WARN !! It seems like the node.js version you are using has changed and might be incompatible with hybrid-metric-framework');
			}
...
/**
 * Sets hybrid-metric-framework tracing functions on the global context
 * @private
 */
...
NJSTrace.prototype.getFormatters = function(formatterConfig, result) {
	...
	// If this is an array, loop thru the array and add it to our results
...
/**
 * The hybrid-metric-framework config object
 * @typedef {object} NJSTrace.NJSConfig
 *
 * @property {boolean} [enabled=true] - Whether tracing is active. Note: hybrid-metric-framework will instrument the code regardless of this setting.
 *
 ...
 *
 * @property {boolean} [wrapFunctions=true] - Whether hybrid-metric-framework should wrap the injected functions with try/catch
 ...
 *
 * @property {boolean} [inspectArgs=true] - Whether hybrid-metric-framework should inspect the traced functions arguments and return values
 *
