...
define(function (require, exports) {
    ...
    let pp = function (v, baseDirToCut, v8Style) {
        if (v.type === 'CalleeVertex')
            //return '\'' + astutil.encFuncName(v.call.attr.enclosingFunction) + '\' (' + astutil.ppPos(v.call) + ')';
            return astutil.ppPos(v.call, baseDirToCut, v8Style);
        if (v.type === 'FuncVertex')
            //return '\'' + astutil.funcname(v.func) + '\' (' + astutil.ppPos(v.func) + ')';
            return astutil.ppPos(v.func, baseDirToCut, v8Style);
        if (v.type === 'NativeVertex')
    ...
    let build = function () {
        ...
        if (args.cg) {
            ...
            cg.edges.iter(function (call, fn) {
                ...
                if (consoleOutput)
                    console.log(pp(call, args.baseDirToCut, args.v8) + " -> " + pp(fn, args.baseDirToCut, args.v8));
            });
    ...
    exports.setFiles = function (inputList, excludeDirs) {
        let filelist = [];
        inputList.forEach(function (file) {
            ...
            else if (fs.statSync(file).isDirectory() && excludeDirs.indexOf(file) === -1) {
