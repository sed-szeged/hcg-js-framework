...
function ppPos(nd, bdir, v8) {
    // if (nd.fake !== undefined) {
    //     return ""
    // }
    let basePath = nd.attr.enclosingFile.replace(bdir, "").replace(/\\/g, "/");
    if (basePath.startsWith("/")) {
        basePath = basePath.substring(1);
    }
    let newCol = v8 ? colNum2V8(Number(nd.loc.start.line), Number(nd.loc.start.column), nd.attr.enclosingFile, nd) : Number(Number(nd.loc.start.column) + 1);
